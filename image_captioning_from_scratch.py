...
class DecoderRNN(nn.Module):
    def __init__(self, embed_size, decoder_dim, attention_dim, vocab_size, device = 'cpu', p = 0.5,):
        ...
        self.lstm = nn.LSTMCell(embed_size, decoder_dim)
        
        self.attention = BasicAttention(2048, decoder_dim, attention_dim)
    ...
    def caption_image(self, features, vocabulary, max_length):
        ...
        hidden, cell = self.init_hidden_state(features)
        # intialize start tokem
        first_word = torch.tensor([vocabulary.stoi['<SOS>']]).view(1,-1).to(self.device)
...
class DecoderRNN(nn.Module):
    ...
    def caption_image(self, features, vocabulary, max_length):
        ...
        hidden, cell = self.init_hidden_state(features)
        # intialize start tokem
        first_word = torch.tensor([vocabulary.stoi['<SOS>']]).view(1,-1)
        embedding = self.embed(first_word)
        # generate words until max_length or EOS token is generated
        for _ in range(max_length):
            # Get attention weights
